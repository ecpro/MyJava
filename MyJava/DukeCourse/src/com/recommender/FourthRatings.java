...
public ArrayList<Rating> getAverageRatingByFilter(int minimalRaters, Filter filterCriteria) {
	...
	public ArrayList<Rating> getSimilarRatings(String id, int numSimilarRaters, int minimalRaters) {
		...
		for(String movie : MovieDatabase.filterBy(new TrueFilter())) {
			...
			if(raterCount >= minimalRaters) finalRatingList.add(new Rating(movie, weightedRating/raterCount));
		}
		
		Collections.sort(finalRatingList, Collections.reverseOrder());
	...
	public static void main(String args []) {
		...
		ArrayList<Rating> recommendations = fr.getSimilarRatings("2", 3, 2);
