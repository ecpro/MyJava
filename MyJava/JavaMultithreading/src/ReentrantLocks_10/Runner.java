...
public class Runner {
    ...
    public void firstThread() throws InterruptedException {
        lock.lock(); // a substitute to sync block
        System.out.println("Waiting ....");
        cond.await(); // same as wait() method
        System.out.println("Woken up!");
        try {
            increment(); 
        } finally {
        	//we put the unlock method in finally block so that the lock get released even if increment() throws an exception
            lock.unlock(); // we must use this statement to release the lock
        }
    }
