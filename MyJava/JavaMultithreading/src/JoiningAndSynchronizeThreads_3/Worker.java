...
public class Worker {
    ...
    /**
     * @author PIYUSH 
     * NOTES : Java objects has only one lock called intrinsic lock or monitor. If any method or block is synchronized then 
     * for a thread to access that method/block has to acquire the lock from the object. Suppose more than one method/block is made 
     * synchronized, in such cases if a thread is accessing one such method/block then other thread cannot access other synchronized 
     * method/block as the key is with the first thread. As soon as the first thread let go of that key, the second thread acquires
     * that key and access the synchronized method/block it wanted to access.
     */
}
